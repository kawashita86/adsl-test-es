...
class ComparaWebService
{
    ...
    public $results;
    private $verify_key_mapping = array(
        'tlcm', 'wind', 'fw', 'tiscali', 'infostrada', 'linkem', 'vdf', 'eolo'
    );
    ...
    public function formatVerifyResults()
    {
        $results_mixed = array();
        foreach ($this->results['result'] as $key => $entry) {
            $key_search = explode('_', $key);
            if (in_array($key_search[0], $this->verify_key_mapping)) {
                if (isset($key_search[1]))
                    $results_mixed['providers'][$key_search[0]][$key_search[1]] = $entry;
                else {
                    $results_mixed['providers'][$key_search[0]]['speed'] = $entry;
                    $results_mixed['providers'][$key_search[0]]['name'] = $key_search[0];
                    $results_mixed['providers'][$key_search[0]]['id_provider'] = $key_search[0];
                }
            } else {
                $results_mixed[$key] = $entry;
            }
        }
        return $this->results = $results_mixed;
    }

    /**
     * @return mixed
     */
    public function mockVerify()
    {
  ...
  "result": {
    ...
    "civico_tlcm_fo": "34",
    "atm_clli": "ROMAITGM",
    "atm_nome_sede_dslam": "VILLA FIORELLI",
    "atm_tipo_dslam": "Standard da centrale",
    "atm_stato": "SATURA",
    "atm_tecnologia": "DSLAM ATM ALCATEL  DSLAM ATM MARCONI",
    "atm_velocita": "20 Mbps",
    "atm_note": "EoS ATM",
    "atm_semaforo_giallo": "NO",
    "eth_clli": "ROMAITGM",
    "eth_nome_sede_dslam": "VILLA FIORELLI",
    "eth_tipo_dslam": "Standard da centrale",
    "eth_stato": "ATTIVA",
    "eth_tecnologia": "ALCATEL ALCATEL",
    "eth_velocita": "20 Mbps",
    "eth_note": "",
    "eth_semaforo_giallo": "NO",
    "lineQuality": {
      "line_type": "ADSL2+",
      "quality": "high",
      "color": "green"
